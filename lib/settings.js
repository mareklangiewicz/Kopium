...
// keep synced with ../pages/redirect.js
...
" - Don't change maps as user ever, always change my settings.js code here, then reload Kopium.
" - Most default vimium keys are very good so prefer them, but change some to more like LazyVim
" - Alt is nicer modifier then Ctrl ? (less conflicts with chrome, consistent with a-1..a-9 for tabs)
...
" All my maps will be explicitly defined here, even though most are like Vimium defaultKeyMappings.
unmapAll
...
" BTW on youtube fullscreen in insert mode there is opposite issue: youtube gets esc before vimium!
...
map fct LinkHints.activateMode action=copy-text
" TODO NOW test these nice actions: hover, focus, copy-text

map fm LinkHints.activateModeToOpenInNewTab
...
" Manipulating tabs
map tn createTab
" BTW It has options like window, incognito, position, and (any-url), test it and maybe define some at least example maps

" Experiment A:
map twc createTab window https://chatgpt.com
map twg createTab window https://grok.com

map H previousTab
map L nextTab
map ^ visitPreviousTab
" TODO does it work across windows too??
" TODO let's try more (neo)vim like maps and maybe remove above map if it's fine
map <c-6> visitPreviousTab
map qb visitPreviousTab
" This is probably nicest/fastest and I kinda treat chrome tabs as buffers anyway (H, L, ..)

" map g0 firstTab
" BTW not needed at all because Ctrl+1 (and same Alt+1)
" map g$ lastTab
" BTW not needed at all because Ctrl+9 (and same Alt+9)
...
map <a-q> removeTab
" Nice one hand key (nicer than c-w) and similar to niri closing apps with Super+Q
...
" map bo closeOtherTabs
" Disabled for now because I too often have too much tabs open,
" and it's dangerous, also I guess don't need it much for now.
